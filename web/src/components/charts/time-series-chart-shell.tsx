...
export interface TimeSeriesChartInnerProps {
  ...
  /** Custom x-axis tick label for a data point (falls back to the short date). */
  tickLabelFormatter?: (point: Record<string, unknown>, index: number) => string;
...
const dateLabels = useMemo(
    () =>
      visiblePlotData.map((d, i) =>
        tickLabelFormatter
          ? (tickLabelFormatter(d, i) ?? shortDateFmt.format(xAccessor(d)))
          : shortDateFmt.format(xAccessor(d)),
      ),
    [visiblePlotData, xAccessor, tickLabelFormatter],
  );
