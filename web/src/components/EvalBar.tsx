...
export default function EvalBar({ wp, label }: EvalBarProps) {
  ...
  return (
    <div className="flex select-none flex-col items-center gap-1">
      <div className="relative h-28 w-2.5 overflow-hidden rounded bg-slate-700 shadow-md border border-line/80">
        <div
          className="absolute inset-x-0 bottom-0 bg-zinc-100 transition-all duration-300"
          style={{ height: `${p}%` }}
        />
        <div className="absolute inset-x-0 top-1/2 h-px -translate-y-1/2 bg-amber-400/80 z-10" />
      </div>
      {label !== undefined && (
